...
def parse_args():
    ...
    parser.add_argument(
        ...
        default="/data/scannet/output/",
    )
    parser.add_argument(
        "--save_name", metavar="DIR", help="file name", default="all_tsdf"
    )
    ...
    # ray multi processes
    parser.add_argument(
        "--n_proc", type=int, default=16, help="#processes launched to process scenes."
    )
...
def save_tsdf_full(
    args, scene_path, cam_intr, depth_list, cam_pose_list, color_list, save_mesh=False
):
    # ======================================================================================================== #
    ...
    for l in range(args.num_layers):
        tsdf_vol_list.append(
            TSDFVolume(
                vol_bnds, voxel_size=args.voxel_size * 2**l, margin=args.margin
            )
        )
    ...
    for id in depth_list.keys():
        ...
        for l in range(args.num_layers):
            tsdf_vol_list[l].integrate(
                color_image, depth_im, cam_intr, cam_pose, obs_weight=1.0
            )
    ...
    for l in range(args.num_layers):
        ...
        np.savez_compressed(
            os.path.join(
                args.save_path, scene_path, "full_tsdf_layer{}".format(str(l))
            ),
            tsdf_vol,
        )
    ...
    if save_mesh:
        for l in range(args.num_layers):
            ...
            meshwrite(
                os.path.join(
                    args.save_path, scene_path, "mesh_layer{}.ply".format(str(l))
                ),
                verts,
                faces,
                norms,
    ...
    for scene in tqdm(scannet_files):
        ...
        if args.dataset == "scannet":
            ...
            intrinsic_dir = os.path.join(
                args.data_path, scene, "intrinsic", "intrinsic_depth.txt"
            )
        ...
        save_tsdf_full(
            args, scene, cam_intr, depth_all, cam_pose_all, color_all, save_mesh=False
        )
...
def generate_pkl(args):
    ...
    for split in splits:
        ...
        for scene in all_scenes:
            ...
            if scene + "\n" in split_files:
                with open(
                    os.path.join(args.save_path, scene, "fragments.pkl"), "rb"
                ) as f:
                    frag_scene = pickle.load(f)
                fragments.extend(frag_scene)

        with open(
            os.path.join(args.save_path, "fragments_{}.pkl".format(split)), "wb"
        ) as f:
            pickle.dump(fragments, f)
